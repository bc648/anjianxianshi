...
- **系统托盘 + 设置面板**：27 项参数（尺寸 / 间距 / 圆角 / 颜色 /
  ...
  受控控件，改动即时生效
- **低开销渲染**：按键只有「按下/松开」两种外观，预渲染成小图缓存，
  每帧只做内存拼接；静止时 CPU ≈ 0%，连点时 ≈ 0.5%，不占 GPU
- **运行日志**：启动 / 退出（含原因）写入本地日志，便于排查；
  滚动保留最近 200 条，长期挂机也不会无限长大
...
> 体积说明：16 MB 里绝大部分是 Python 运行时和 Pillow。不用 UPX 压缩——
> 加壳会显著增加安全软件误报风险，省下的体积不值得。

## 性能

实测（1920×1080、窗口 195×291）：

| 场景 | CPU | 内存 | GPU |
|---|---|---|---|
| 静止挂机 | ≈ 0% | ~21 MB | 0 |
| 连点 39 次/秒 | ≈ 0.5% | ~21 MB | 0 |
| 长期挂机 | 稳定，无内存/句柄增长 | 同上 | 0 |
